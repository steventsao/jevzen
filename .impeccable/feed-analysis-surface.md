...
Shipping boundary: popup.html and settings.html show only rules, provider/display controls, and groups from the actual X tab. No synthetic feed, sample composer, lab navigation, or localhost dependency ships. The optional developer lab builds separately into dev-dist/. Story selections focus posts on X itself.

## Direction contract

THESIS: A reading lens with visible, adjustable attenuation. Post text leads; model measurements stay beside the decision they explain.

OWN-WORLD: Cool off-white surfaces, deep blue-black ink, cobalt actions, compact workhorse typography, fine separators, square-ended measurement tracks, rounded action buttons. A sidebar behaves like a small instrument panel. The X overlay inherits the host theme.

STORY: Connect a personal key, edit one rules document, Apply to update the feed, see real probabilities, adjust fading, reveal any post, inspect topic or story groups. Typing is a draft; the applied rules remain inspectable until Apply succeeds.
