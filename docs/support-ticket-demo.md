# TypeSafe with Effect: support-ticket demo

This is the original example in the jevzen repository. Run the commands below
from the repository root. For the Chrome extension, see the [main README](../README.md).
...
Requires Node.js 22.9+ and pnpm. Dependency versions are pinned in the lockfile.
Copy `.env.example` to `.env` and supply your `TYPESAFE_API_KEY`; `.env` is ignored by Git.
...
# Original example using the official TypeSafe SDK.
pnpm original
```
